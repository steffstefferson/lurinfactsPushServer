...
const firebase = {
    getPushState: function () { return true; },
    updatePushState: function (state) { return true; },
    getNewestFact: function () { return Promise.resolve(null); },
    getNewestImage: function () { return Promise.resolve(null); }
};

var msgTrigger = require("../pushMessageTrigger.js");
...
var helper = {
    provideImage: function (insertTimeOfImage) {
        return function (t) {
            ...
            return Promise.resolve({ img: 'url', insertTime: insertTimeOfImage || new Date() });
        };
    },
    provideFact: function (inserTimeOfFact) {
        return function (t) {
            ...
            return Promise.resolve({ fact: 'imafact', insertTime: inserTimeOfFact || new Date() });
        };
    }
};
...
describe('#CheckGetLatestContent', function () {
    ...
    it('has a new fact content', function () {
        ...
        return triggerLogic.getNewContentToPush({ lastFactDate: yesterday }).then(function (content) {
    ...
    it('has a fact already pushed', function () {
        ...
        return triggerLogic.getNewContentToPush({ lastFactDate: tomorrow }).then(function (content) {
            assert.equal(null, content);
        });
    });
    ...
    it('has a new image content', function () {
        firebase.getNewestFact = function () { return Promise.resolve(null); };
        firebase.getNewestImage = helper.provideImage();
        var yesterday = new Date();
        yesterday.setDate(yesterday.getDate() - 1);
        return triggerLogic.getNewContentToPush({ lastImageDate: yesterday }).then(function (content) {
            assert.notEqual(null, content);
    ...
    it('has a image already pushed', function () {
        ...
        var tomorrow = new Date();
        tomorrow.setDate(tomorrow.getDate() + 1);
        return triggerLogic.getNewContentToPush({ lastImageDate: tomorrow }).then(function (content) {
            assert.equal(null, content);
        });
    });
