...
describe("#CheckTimeToRun", function () {
  ...
  it("should run now, one day passed", function () {
    ...
    assert.equal(true, isTime.isTimeToPushNew);
    assert.equal(true, isTime.isTimeToPushRandom);
    assert.equal(true, isTime.isTimeToRun);
  });

  it("should run radom fact, but not new content", function () {
    var result = new Date();
    var oneDayInMs = 3600 * 25 * 1000;
    result = new Date(result.getTime() - oneDayInMs);
    var isTime = triggerLogic.isTimeToRun(
      { lastPushMessage: result, pushNewContentEveryXHours: 400 },
      true
    );
    assert.equal(false, isTime.isTimeToPushNew);
    assert.equal(true, isTime.isTimeToPushRandom);
    assert.equal(true, isTime.isTimeToRun);
  });
